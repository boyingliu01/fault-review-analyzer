...
from src.embedding.models import EmbeddingResult, BatchEmbeddingResult
...
class TestEmbeddingGeneratorBoundary:
    ...
    async def test_embed_batch_local_provider(self):
        ...
        assert len(result) == 3
        assert all(len(emb) == 384 for emb in result)  # 本地嵌入维度是 384
    ...
    def test_get_dimension_known_models(self):
        """测试已知模型的维度"""
        generator = EmbeddingGenerator(provider="openai", api_key="test-key", model="text-embedding-3-small")
        assert generator.get_dimension() == 1536
