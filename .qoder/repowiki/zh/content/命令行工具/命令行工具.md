...
U->>CLI : fault-analyzer fetch|analyze|report ...
CLI->>F : 解析参数并执行
F->>C : 加载配置
F-->>U : 进度/成功/失败提示
CLI->>A : 解析参数并执行
...
## 结论
本命令行工具围绕“获取-分析-报告”的核心链路，提供了完善的 CLI 能力与灵活的配置体系。通过合理的缓存策略、参数调优与日志诊断，可在生产环境中稳定高效地支撑大规模故障复盘与聚类分析。
