...
## 更新摘要
**变更内容**
- 新增 Git Hooks 集成配置章节，说明 .pre-commit-config.yaml 的配置和使用方法
- 更新项目结构章节，包含新的 Git 配置文件和钩子脚本
- 增强开发环境最佳实践，包含代码质量检查和自动化流程
- 更新配置安全管理章节，增加对生成文件和敏感信息的处理建议
...
- Embedding 配置
  - provider: 枚举(openai/bge/m3e/codebert/zhipu/local/volcengine/custom)
  - model: 字符串，必填
  - api_key: 非 local 时必填
  - base_url: 可选，http(s) 格式
  - batch_size: 整数，>0
...
## 结论
该配置管理系统以 Pydantic 模型为核心，结合 YAML/JSON 文件与环境变量覆盖，提供强类型、可校验、易扩展的配置能力。通过 CLI 命令与完善的错误提示，便于开发与运维在不同环境中高效管理与维护配置。新增的 Git Hooks 集成进一步提升了代码质量和开发体验。
