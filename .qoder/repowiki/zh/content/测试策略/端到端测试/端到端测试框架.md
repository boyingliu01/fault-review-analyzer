...
- [tests/e2e/api/test_user_workflows.py](file://tests/e2e/api/test_user工作流.py)
...
subgraph "API 测试"
A1["api/test_server_lifecycle.py"]
A2["api/test_user_workflows.py"]
end
subgraph "CLI 测试"
L1["cli/test_cli_workflow.py"]
L2["cli/test_analyze.py"]
L3["cli/test_fetch.py"]
L4["cli/test_report.py"]
end
subgraph "流水线测试"
P1["pipeline/test_phase1.py"]
P2["pipeline/test_phase2.py"]
P3["pipeline/test_full_pipeline_real.py"]
end
subgraph "UI 测试"
U1["ui/test_streamlit.py"]
end
R --> A1
R --> A2
R --> L1
...
## 结论
本端到端测试框架以 pytest 为核心，结合 Playwright 与丰富的夹具体系，覆盖了 API、CLI、UI 与流水线等关键入口。通过统一的运行入口与清晰的模块化组织，既保证了测试的可维护性，也提升了执行效率与稳定性。建议在持续集成环境中常态化运行，并结合产物快照与日志归档，进一步提升回归检测能力。
