...
def task_info() -> TaskInfo:
    return TaskInfo(
        ...
        description="Task used to exercise pipeline cache I/O",
        status="resolved",
        priority="medium",
...
def pipeline() -> AnalysisPipeline:
    return AnalysisPipeline(MagicMock(), PipelineConfig(use_cache=True))


@pytest.mark.asyncio
async def test_fetch_task_loads_cache_without_blocking_event_loop(
    ...
    pipeline._cache_manager = cache

    fetch = asyncio.create_task(pipeline._fetch_task(task_info.task_id))
    try:
...
@pytest.mark.asyncio
async def test_fetch_task_saves_cache_without_blocking_event_loop(
    pipeline: AnalysisPipeline,
    task_info: TaskInfo,
) -> None:
    loop = asyncio.get_running_loop()
